...
A single historical backtest is insufficient to establish strategy robustness \cite{chan2018}. We adopt the simulation-based validation framework proposed by Chan (2018).
...
% ============================================
% FIGURE 4: ATTENTION HEATMAP COMPARISON
% ============================================
% DEV TODO: Generate a side-by-side attention heatmap showing:
%   - Left panel: Pre-COVID (e.g., Jan 2020) attention matrix A_{ij}
%   - Right panel: COVID crash peak (e.g., Mar 2020) attention matrix A_{ij}
%   - Rows/columns = 33 assets (grouped by GICS sector)
%   - Color scale: attention weight magnitude (0 to max)
%   - Highlight how defensive sectors (Utilities, Telecom) gain attention during crisis
%   - Optionally add a third panel for 2022 inflation shock
% Save as: attention_heatmap.png
\begin{figure}[H]
\centering
% \includegraphics[width=\columnwidth]{attention_heatmap.png}
\fbox{\parbox{0.95\columnwidth}{\centering\vspace{2.5cm}\textbf{PLACEHOLDER: Attention Heatmap Comparison}\\[0.5em]
\small Left: Pre-COVID (Jan 2020) \quad Right: COVID crash (Mar 2020)\\Assets grouped by GICS sector. Show attention shift to defensive sectors.\vspace{2.5cm}}}
\caption{GAT attention weights $\alpha_{ij}$ before and during the COVID-19 crash. Assets are grouped by GICS sector. During the crisis, the model autonomously upweights connections involving defensive sectors (Utilities, Telecom) and downweights cyclical sectors (Energy, Consumer Discretionary).}
