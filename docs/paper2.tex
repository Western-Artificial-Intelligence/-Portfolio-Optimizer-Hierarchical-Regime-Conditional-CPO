...
AdamW optimizer (lr $= 10^{-3}$, weight\_decay $= 10^{-4}$) with cosine annealing learning rate schedule over 50 epochs. Early stopping with patience 10 on validation Sharpe ratio. The final model per fold is the checkpoint achieving maximum validation Sharpe.
...
We hypothesize the GNN Supervisor will:
\begin{enumerate}
    \item \textbf{Match or exceed} the XGBoost Supervisor on risk-adjusted metrics over the full 2021--2024 test period, benefiting from relational market structure information unavailable to a tabular model.
    \item \textbf{Outperform significantly} the XGBoost Supervisor during regime transition periods (late February 2020, January 2022), where inter-asset contagion dynamics dominate and tabular features lag the actual propagation.
    \item \textbf{Demonstrate emergent interpretability:} learned attention weights should concentrate on economically meaningful sector pairs and shift toward defensive-defensive connections during crises—without any explicit regime labeling.
    \item \textbf{Maintain robustness:} the synthetic validation (1{,}000 bootstrapped histories) Sharpe distribution should be right-shifted versus both the Worker-only and XGBoost Supervisor baselines, confirming that the GNN learns generalizable regime structure.
\end{enumerate}

This research contributes to the field by demonstrating that \emph{dynamic graph topology}—where the model learns which inter-asset relationships matter in which market conditions—provides information beyond what is accessible to any tabular supervisor operating on marginal feature distributions. The hierarchical separation of concerns (QP math for construction, GNN for relational risk management) provides a principled, end-to-end trainable, and regime-interpretable approach to adaptive portfolio optimization.
